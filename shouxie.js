...
//    console.log(event)
      ...
      if (event.type.indexOf('touch') !== -1) { // event.constructor === TouchEvent
        xPos = (event.touches[0].clientX - rect.left) * (this.canvas.width/rect.width);
        
        yPos = (event.touches[0].clientY - rect.top) * (this.canvas.height/rect.height);
      }
      // Mouse event
      else {
        /*xPos = event.clientX - rect.left;
        yPos = event.clientY - rect.top;*/
        xPos = (event.clientX - rect.left) * (this.canvas.width/rect.width);
        yPos = (event.clientY - rect.top) * (this.canvas.height/rect.height);
